...
const Internship = () => {
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = () => {
      ...
      if (swipeDistance > 50 && index < internships.length - 1) {
        setDirection(1);
        setIndex((prev) => prev + 1);
      } else if (swipeDistance < -50 && index > 0) {
        setDirection(-1);
        setIndex((prev) => prev - 1);
      }
    };
  ...
  return (
    <section
  id="internship"
  className="min-h-screen flex flex-col items-center justify-start px-4 sm:px-6 lg:px-12 pt-10 sm:pt-14 pb-16 relative overflow-hidden text-gray-800 dark:text-white"
>
      {/* Background */}
      <div className="absolute inset-0 bg-gradient-to-br from-blue-100 via-purple-100 to-pink-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900"></div>
      ...
      {/* Content */}
      <div className="relative z-10 w-full max-w-5xl">
        <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-center">Internships</h2>

        <div className="relative w-full h-auto sm:h-[500px]">
          <AnimatePresence custom={direction} mode="wait">
            <motion.div
              key={index}
              custom={direction}
              ...
              animate="animate"
              exit="exit"
              className="absolute w-full h-auto sm:h-full bg-white dark:bg-gray-900 rounded-xl shadow-xl p-6 sm:p-8 flex flex-col md:flex-row gap-6 sm:gap-8 items-center justify-between"
            >
              {/* Image */}
              <div className="w-full md:w-1/2 flex justify-center">
                <img
                  src={internship.image}
                  alt={internship.title}
                  className="max-h-60 sm:max-h-72 rounded-lg shadow-md object-cover"
                />
              </div>
              ...
              <div className="w-full md:w-1/2 text-center md:text-left">
                ...
                <h4 className="text-sm sm:text-lg italic font-semibold text-indigo-500 mt-1">{internship.role}</h4>
                <p className="text-xs sm:text-sm mt-2 text-gray-600 dark:text-gray-300">{internship.duration}</p>
